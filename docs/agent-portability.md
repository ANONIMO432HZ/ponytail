...
| Antigravity IDE | `scripts/antigravity.js`, `hooks/antigravity-hooks.json`, `skills/`, `.agents/rules/ponytail.md` | Full customization installer: `node scripts/antigravity.js install` sets up the always-on rule, the six native skills, and the `PreInvocation` hook (`ponytail-mode-tracker.js`) in `~/.gemini/config/` (or `<cwd>/.agents/` with `--project`). Also auto-reads `AGENTS.md` at repo root as zero-setup fallback. Contract and verification: [antigravity.md](antigravity.md). |
